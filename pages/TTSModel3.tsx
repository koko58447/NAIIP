...
import { generateSpeechModel3And4, TTSResult, Gender } from '../services/ttsService';
import { useTranslation } from '../contexts/LanguageContext';

interface TTSModel3Props {
    onNavigateBack: () => void;
}

const TTSModel3: React.FC<TTSModel3Props> = ({ onNavigateBack }) => {
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            const response = await generateSpeechModel3And4(params);
            setResult(response);
        } catch (e: any) {
    ...
    }, [inputText, isLoading, gender, removeSilence, speed, phraseLimit, t]);
        ...
        <div className="flex flex-col items-center p-4 sm:p-6 lg:p-8">
            <main className="w-full max-w-3xl mx-auto space-y-6">
                ...
                <div className="bg-gray-800/50 p-4 rounded-xl border border-gray-700 space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-300 text-center mb-2">{t('ttsGender')}</label>
                        <fieldset className="flex justify-center gap-x-6">
                            <div className="flex items-center">
                                <input id="gender-male" name="gender" type="radio" value="male" checked={gender === 'male'} onChange={() => setGender('male')} className="h-4 w-4 border-gray-600 bg-gray-700 text-cyan-600 focus:ring-cyan-500" />
                                <label htmlFor="gender-male" className="ml-2 block text-sm font-medium leading-6 text-gray-300">{t('ttsMale')}</label>
                            </div>
                            <div className="flex items-center">
                                <input id="gender-female" name="gender" type="radio" value="female" checked={gender === 'female'} onChange={() => setGender('female')} className="h-4 w-4 border-gray-600 bg-gray-700 text-cyan-600 focus:ring-cyan-500" />
                                <label htmlFor="gender-female" className="ml-2 block text-sm font-medium leading-6 text-gray-300">{t('ttsFemale')}</label>
                            </div>
                        </fieldset>
                    </div>
